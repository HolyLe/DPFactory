...
@interface DPCommonNavigationBar()
...
@implementation DPCommonNavigationBar
...
- (void)setDelegate:(id<DPCommonNavigationBarDelegate>)delegate{
    if (!delegate) return;
    _delegate = delegate;
...
- (UIButton *)middleButton{
    if (!_middleButton) {
        ...
        .makeMasonry(^(MASConstraintMaker * _Nonnull make) {
            ...
            make.width.lessThanOrEqualTo(@([UIScreen mainScreen].bounds.size.width - 200 *DP_SCALE));
            make.top.mas_offset(kStatusBarHeight);
...
- (UIView *)rightView{
    if (!_rightView) {
        ...
        .makeMasonry(^(MASConstraintMaker * _Nonnull make) {
            ...
            make.bottom.equalTo(self.mas_bottom).offset(0.5);
            make.top.mas_offset(kStatusBarHeight);
            make.width.lessThanOrEqualTo(@(100 *DP_SCALE));
        });
